...
class VoiceNotesApp {
  // FIX: Correctly type the genAI client.
  ...
  constructor() {
    // FIX: Updated GoogleGenAI initialization to use API_KEY and remove deprecated apiVersion.
  ...
  private async getTranscription(
    ...
    try {
      ...
      if (transcriptionText) {
        if (this.currentNote) {
          if (this.currentNote.rawTranscription.trim() === '') {
  ...
  private async getPolishedNote(transcriptionToPolish: string): Promise<void> {
    try {
      if (!transcriptionToPolish || transcriptionToPolish.trim() === '') {
        this.recordingStatus.textContent = 'No new transcription to polish.';
        return;
      }
      ...
      if (polishedText) {
        let fullPolishedNote = '';
        if (this.currentNote) {
          if (this.currentNote.polishedNote.trim() === '') {
            this.currentNote.polishedNote = polishedText;
          } else {
            this.currentNote.polishedNote += '\n\n' + polishedText;
          }
          fullPolishedNote = this.currentNote.polishedNote;
        } else {
          fullPolishedNote = polishedText;
        }

        // FIX: Use marked.parse() to convert markdown to HTML, as `marked` is an object and not a function.
        const htmlContent = await marked.parse(fullPolishedNote);
        this.polishedNote.innerHTML = htmlContent;

        if (fullPolishedNote.trim() !== '') {
  ...
  private async exportNote(): Promise<void> {
    const polishedContent = this.currentNote?.polishedNote;
    ...
    if ('showSaveFilePicker' in window) {
      try {
        ...
        await writable.write(polishedContent);
        ...
        if (err instanceof Error && err.name === 'AbortError') {
          ...
          this.fallbackExport(polishedContent, fileName);
        }
      }
    } else {
      this.fallbackExport(polishedContent, fileName);
  ...
  private fallbackExport(content: string, fileName: string): void {
